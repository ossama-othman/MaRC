...
template <typename T>
MaRC::MapCommand_T<T>::MapCommand_T (std::string const & filename,
...
MaRC::MapCommand_T<T>::initialize_FITS_image (fitsfile * fptr, int & status)
{
   ...
   */
  fits_create_img(fptr, FITS::traits<T>::bitpix, naxis, naxes, &status);
...
MaRC::MapCommand_T<T>::make_map_planes (fitsfile * fptr, int & status)
{
  ...
  int plane_count = 1;
  size_t const num_planes = this->image_factories_.size ();
